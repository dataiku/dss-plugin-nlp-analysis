...
Token.set_extension(
    "is_username", getter=lambda token: token.text[0] == "@", force=True
)
Token.set_extension(
    "is_emoji",
...
ALL_UNITS = (
    ORDER_UNITS | WEIGHT_UNITS | DISTANCE_SPEED_UNITS | VOLUME_UNITS | MISC_UNITS
)
Token.set_extension(
    "is_measure",
    getter=lambda token: not token.like_num  # avoid conflict with existing token attribute
    ...
    and any(
        [
            re.sub(NUMERIC_SEPARATOR_REGEX, "", token.lower_)
            .replace(unit, "")
            .isdigit()
...
class MultilingualTokenizer:
    ...
    def tokenize_list(self, text_list: List[AnyStr], language: AnyStr) -> List[Doc]:
        ...
        start = perf_counter()
        logging.info(
            f"Tokenizing {len(text_list)} document(s) in language '{language}'..."
        )
    ...
    def tokenize_df(
        ...
        df[self.tokenized_column] = pd.Series(
            [Doc(Vocab())] * len(df.index), dtype="object"
        )
